...
export const NoteEditor = ({ noteId, onTogglePanel, isPanelOpen, onSelectNote, onAiClick }: NoteEditorProps) => {
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({ placeholder: "Start writing... (Tip: use '$' to link notes)" }),
      Underline,
  ...
  const { openFilePicker, openImagePicker, fileInputRef, imageInputRef, handleFileChange, handleImageChange } = useFileAttachment({
    ...
    onPDFAttach: (_dataUrl: string, _fileName: string, _fileSize: number) => {},
    onCSVAttach: (_dataUrl: string, _fileName: string, _fileSize: number) => {},
    onExcelAttach: (_dataUrl: string, _fileName: string, _fileSize: number) => {},
  });
  ...
  return (
    <div className="flex-1 flex flex-col bg-background h-full overflow-hidden relative">
      <input ref={fileInputRef} type="file" accept=".pdf,.doc,.docx,.txt,.csv,.xlsx,.pptx,image/*" onChange={handleFileChange} className="hidden" multiple />
      <input ref={imageInputRef} type="file" accept="image/*" onChange={handleImageChange} className="hidden" multiple />
      ...
      <div className="h-12 flex items-center justify-between px-4 shrink-0 border-b border-border/50">
        ...
        <div className="flex items-center gap-1 shrink-0">
          <div className="hidden sm:flex items-center px-2">
            <span
              className={cn(
                ...
                isDirty ? 'bg-amber-500/10 text-amber-600 dark:text-amber-400' : 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400'
              )}
            >
              <Clock size={10} />
          ...
          <div className="hidden sm:flex items-center gap-1">
            <Button variant="ghost" size="sm" className="h-9 gap-1.5 text-muted-foreground" onClick={handleShare} title="Share this note">
              <Share2 size={16} /> Share
            </Button>
            <Button variant="ghost" size="sm" className="h-9 w-9 text-muted-foreground"
              onClick={() => {
                togglePin(note.id);
            ...
            <Button variant="ghost" size="sm" className="h-9 w-9 text-primary opacity-70" onClick={handleAiSuggest} title="AI Assistant Suggestion">
              <Sparkles size={16} />
            </Button>
            <div className="w-px h-4 bg-border mx-1" />
            <Button variant={isDirty ? 'default' : 'ghost'} size="sm" onClick={handleSave} disabled={!isDirty} className="h-9 gap-2" title="Save version history (Commit)">
              <Save size={16} />
              <span className="hidden sm:inline">Commit</span>
            </Button>
            <Button variant="ghost" size="sm" className="h-9 w-9 text-muted-foreground hover:bg-accent rounded-lg" onClick={onTogglePanel} title={isPanelOpen ? "Close side panel" : "Open side panel"}>
              {isPanelOpen ? <PanelRightClose size={18} /> : <PanelRight size={18} />}
            </Button>
            <Button variant="ghost" size="icon" className="h-9 w-9 text-muted-foreground" onClick={handleDelete} title="Delete this note">
              <Trash2 size={16} />
            </Button>
          </div>
        </div>
      ...
      <div className="relative">
        <EditorToolbar editor={editor} onEmojiClick={() => setShowEmojiPicker(prev => !prev)} onLinkClick={() => setShowLinkDialog(true)} onImageClick={openImagePicker} onFileClick={openFilePicker} onGoLiveClick={handleGoLive} isLiveMode={isLiveMode} onlineUsersCount={onlineUsersCount} />
      ...
      <div className="flex-1 overflow-y-auto w-full">
        <div className="max-w-3xl mx-auto px-6 sm:px-12 pt-12 pb-32">
          <div className="mb-4 opacity-0 hover:opacity-100 transition-opacity group flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg border-2 border-dashed border-border flex items-center justify-center hover:border-primary/50 hover:bg-accent transition-all cursor-pointer text-muted-foreground" onClick={handleAddBlock} title="Add new content block">
              <Plus size={18} />
            </div>
          </div>

          <textarea value={title} onChange={e => setTitle(e.target.value)} onBlur={() => { if (note && title !== note.title) { renameNote(note.id, title); toast.success('Note renamed'); } }} rows={1} placeholder="Untitled" className="w-full text-3xl sm:text-4xl md:text-5xl font-extrabold text-foreground bg-transparent border-none focus:ring-0 resize-none outline-none mb-6 placeholder:text-muted-foreground/20 leading-[1.1] transition-all" style={{ minHeight: '1.2em' }} />
            ...
            <div className="flex items-center gap-1.5 min-w-0">
              ...
              <div className="flex gap-1 flex-wrap items-center">
                {note.tags.map(t => {
                  ...
                  return (
                    <span key={t} className="group/tag inline-flex items-center gap-1.5 text-[11px] px-2.5 py-0.5 rounded-full font-bold transition-all border shrink-0" style={{ backgroundColor: (tagInfo?.color || '#8b5cf6') + '15', color: tagInfo?.color || '#8b5cf6', borderColor: (tagInfo?.color || '#8b5cf6') + '30' }}>
                      <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: tagInfo?.color || '#8b5cf6' }} />
                      {t}
                      <button onClick={() => handleRemoveTag(t)} className="opacity-0 group-hover/tag:opacity-100 p-0.5 hover:bg-black/5 rounded-full transition-all" title={`Remove tag: ${t}`}>
                ...
                {isAddingTag ? (
                  <div className="relative animate-in fade-in zoom-in-95 duration-200">
                    <input autoFocus value={tagSearch} onChange={e => setTagSearch(e.target.value)} placeholder="Search tags..." className="text-[11px] bg-accent/50 border border-border rounded-full px-3 py-1 outline-none w-32 focus:w-40 transition-all shadow-sm" />
                    <div className="absolute top-full left-0 mt-2 w-48 bg-card border border-border rounded-xl shadow-2xl z-[100] overflow-hidden max-h-60 overflow-y-auto premium-shadow">
                      <div className="p-1.5 space-y-0.5">
                        {globalTags.filter(t => t.name.toLowerCase().includes(tagSearch.toLowerCase()) && !note.tags.includes(t.name)).map(t => (
                          <button key={t.id} onClick={() => handleAddTag(t.name)} className="w-full flex items-center gap-2 px-2.5 py-1.5 hover:bg-accent rounded-lg transition-colors text-left">
                      ...
                      <div className="bg-muted/30 p-1.5 border-t border-border">
                         <button onClick={() => { setIsAddingTag(false); setTagSearch(''); }} className="w-full py-1 text-[10px] font-bold text-muted-foreground hover:text-foreground hover:bg-accent rounded transition-colors uppercase tracking-tight">Close</button>
                      </div>
                    </div>
                  </div>
                ) : (
                  <button onClick={() => setIsAddingTag(true)} className="p-1 hover:bg-accent rounded-full text-muted-foreground/40 hover:text-primary transition-all" title="Add new tag">
                    <Plus size={12} />
                  </button>
                )}
              </div>
      ...
      <LinkInsertDialog isOpen={showLinkDialog} onClose={() => setShowLinkDialog(false)} onInsert={handleLinkInsert} initialText={editor?.state.selection.empty ? '' : editor?.state.doc.textBetween(editor.state.selection.from, editor.state.selection.to) || ''} />
      
      {/* Collaboration Indicator removed for now to fix JSX issues */}
      
      <AnimatePresence>
        {isCommitting && (
          <div className="absolute inset-0 z-50 flex items-center justify-center p-6 bg-background/60 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.92, opacity: 0 }} className="w-full max-w-md bg-card border border-border shadow-2xl rounded-2xl overflow-hidden">
              <div className="p-6">
                <h3 className="font-bold text-lg mb-4">Save version</h3>
                <textarea autoFocus value={commitMessage} onChange={e => setCommitMessage(e.target.value)} placeholder="What changed?" className="w-full h-24 p-4 rounded-xl bg-accent/50 border border-border text-sm outline-none resize-none" />
