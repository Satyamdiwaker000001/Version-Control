      ...
      <div className="space-y-4 flex-1 overflow-y-auto">
        {comments.map(comment => (
          <div key={comment.id} className="flex items-start space-x-3">
            <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center font-bold text-sm" style={{ backgroundColor: MOCK_TEAM_MEMBERS.find(m => m.id === comment.author.id)?.color + '30', color: MOCK_TEAM_MEMBERS.find(m => m.id === comment.author.id)?.color }}>
